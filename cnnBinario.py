...
# Filtrar dígitos 0 e 1
indices_treino_0 = np.where(y_treino == 0)
indices_treino_1 = np.where(y_treino == 1)
indices_teste_0 = np.where(y_teste == 0)
indices_teste_1 = np.where(y_teste == 1)

x_treino = np.concatenate((x_treino[indices_treino_0], x_treino[indices_treino_1]))
y_treino = np.concatenate((np.zeros(len(indices_treino_0[0])), np.ones(len(indices_treino_1[0]))))

x_teste = np.concatenate((x_teste[indices_teste_0], x_teste[indices_teste_1]))
y_teste = np.concatenate((np.zeros(len(indices_teste_0[0])), np.ones(len(indices_teste_1[0]))))

# Pré-processamento dos dados
x_treino = x_treino.reshape(-1, 28, 28, 1) / 255.0
x_teste = x_teste.reshape(-1, 28, 28, 1) / 255.0
y_treino = tf.keras.utils.to_categorical(y_treino, num_classes=2)
y_teste = tf.keras.utils.to_categorical(y_teste, num_classes=2)
